...
//  messagingApp
//
//  Created by 90308616 on 5/18/22.
//
...
class ChatroomsViewModel: ObservableObject{
        ...
        if(user != nil){
            db.collection("chatrooms").whereField("users", arrayContains: user!.uid).addSnapshotListener({(snapshot, error) in
                ...
                self.chatrooms = documents.map({docSnapshot -> Chatroom in
                    ...
                    let title = data["title"] as? String ?? "New Chatroom"
                    let joinCode = data["joinCode"] as? Int ?? -1
                    let userId = self.user!.uid
                    return Chatroom(id: docId, title: title, joinCode: joinCode, userId: userId)
    ...
    func joinChatroom(code: String, handler: @escaping () -> Void){
        if(user != nil){
            db.collection("chatrooms").whereField("joinCode", isEqualTo: Int(code) ?? 1111).getDocuments() { (snapshot, error) in
                if let error = error {
